...
zipperwrapper  <- function(nveh1,nveh2,umn,usd,xstart1,xstart2,step,tstart,tend,xfunnel,leff,type,browse) {
  ...
  df1df2 <- zippermerge(nveh, tstart, tend, xstart, umn, leff, xfunnel, step, type)
  print(head(df1df2))
#  browser()
  ...
  if(type == TRUE) {
    ...
    axis(side = 3, at = k.a.mn, label = expression(bar(k)[A]), tick = FALSE, line = -1)
    axis(side = 3, at = k.d.mn, label = expression(bar(k)[D]), tick = FALSE, line = -1)
