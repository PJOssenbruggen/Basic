...
#' @usage merge3(df1,df2,leff,step,xfunnel,usd,ylim,type)
# #' @examples
# #' merge3(df1,df2,leff,step,xfunnel,usd,ylim,type)
#' @export
merge3 <- function(df1, df2,leff,step,xfunnel,usd,ylim,type) {
  if(type == TRUE) {
  ...
  df13    <- df1[df1[,1] >= t1,]
  if(type == TRUE) lines(df13[,1], df13[,3], col = "yellow", lty = 4, lwd = 3)
  if(type == TRUE) lines(tux3[,1], tux3[,3], col = "yellow", lty = 4, lwd = 3)
  for(i in 1:1) {
