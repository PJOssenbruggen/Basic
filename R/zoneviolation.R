...
#' @usage zoneviolation(veh, zone, df1df2, dfcrit, step, tend.0, leff)
# #' @examples
# #' zoneviolation(veh, zone, df1df2, dfcrit, step, tend.0, leff)
#' @export
zoneviolation <- function(veh, zone, df1df2, dfcrit, step, tend.0, leff) {
  ...
  df     <- findviolation(tstart, tend, tend.0, df1, df2, step, leff)
  X      <- sum(df[,1])
  ncol   <- 5 + zone
  dfcrit[veh,ncol] <- X
  return(dfcrit)
