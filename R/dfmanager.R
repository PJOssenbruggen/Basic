...
#' @usage dfmanager(nveh, tend, umn, usd, xstart, xfunnel, leff, lane, delt)
# #' @examples
# #' dfmanager(nveh = 3, tend = 16, umn = 41, usd = 11, xstart = c(-600,-750,-1000), xfunnel = -500, leff = 14, lane = c(1,2,1), delt = 2)
#' @export
dfmanager <- function(nveh, tend, umn, usd, xstart, xfunnel, leff, lane, delt) {
  set.seed(127)
  tend.save <- tend
  ...
  veh <- 2
  df <- vehtfix(veh, nveh, df, tstart., tend., delt, ufix, xfix)
  return(df)
}
